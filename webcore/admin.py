...
class AdAdmin(admin.ModelAdmin):
    ...
    def publish(self, request, queryset):
        ...
        self.message_user(request, f"{updated} объявлений опубликовано", messages.SUCCESS)

    @admin.action(description='Снять с публикации объявления')
    def unpublish(self, request, queryset):
        updated = queryset.update(is_published=False)
        self.message_user(request, f"{updated} объявлений снято с публикации", messages.SUCCESS)
